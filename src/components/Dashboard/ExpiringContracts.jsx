import { Table, Space, Typography, Avatar, Button } from "antd";
import { ClipboardDocumentCheckIcon, ArrowRightCircleIcon, ArrowLeftCircleIcon } from "@heroicons/react/24/outline";
import { useEffect, useState } from "react";
import config from "../../config";

const { Text } = Typography;

export default function ExpiringContracts({ showViewMore }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const contracts = result?.get_expiring_contracts?.data || [];

        // Normalize format
        const transformed = contracts.map((item, index) => ({
          id: item.id || `contract-${index}`,
          client: item.client,
          accManager: item.accManager,
          avatar: item.avatar,
  ...
  const columns = [
    ...
    {
      title: isSmallScreen ? "Exp.Date" : "Expiration Date",
    ...
    {
      title: isSmallScreen ? null : "Actions",
      key: "actions",
      width: "30%",
      align: "center",
      render: () => (
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 h-full flex flex-col">
      <h2 className="text-lg font-semibold mb-4">Contracts Expiring Soon</h2>
